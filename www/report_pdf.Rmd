...
header-includes:
 ...
 - \pagestyle{fancy}
 - \fancyhead[]{}
 - \fancyfoot[LO,LE]{\footnotesize Extended Smooth Pathway Model report - `r format(Sys.time(), "%d %b %Y")`}
 - \fancyfoot[C]{}
...
input_data <- tibble(
  "Parameters" = c("Global emission budget", "Global budget from 2020 on", "Weighting \"population\"", "EU emission budget from 2020 on", "Max. possible net neg. emissions"),
  "Value" = c(paste0(params$global_emission_budget_gt_2018, " Gt CO2"), 
              ...
              paste0(round(params$eu_emission_budget_gt, 1), " Gt CO2"),
              paste0(round(params$max_negative_emissions_gt, 1), " Gt CO2")
              )
)

title <- textGrob("Input parameters", y = unit(0.75,"npc"), x = unit(0.25, "npc"), 
...
plot <- params$result %>%
  ...
  # annotation_custom(tableGrob(params$overshoot_amounts, rows = NULL, theme = ttheme_minimal(base_size = 6, padding = unit(c(2, 2), "mm"))), xmin = ifelse(date_display_range() == 2100, 2060, 2035), ymin = 1.5, ymax = 3) +
  theme_classic() +
...
grid.arrange(gt, plot, ncol = 2, widths = c(1.4, 2.3))
```
