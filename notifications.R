...
output$title <- renderUI({
  output = tagList()
  output[[1]] <- HTML("Smooth Emission Paths<br><span style = 'font-size:20px;'>Calculate emission paths in accordance to a freely adjustable emission budget 2020 - 2100</span>")
...
output$box_info_scenario_type <- renderUI({
  hidden(div(class = "info-box", style = "left:330px;", id = "info_scenario_type", HTML("Scenario types differ regarding the annual 
               ...
               <ul><li><a href = 'https://www.klima-retten.info/Downloads/RM-Scenario-Types_short.pdf', target = '_blank'>Brief description</a></li>
...
output$box_info_budget <- renderUI({
  hidden(div(class = "info-box", style = "left:390px; width:650px;", id = "info_budget", 
             HTML("
Please ensure that budget and base year emissions refer to the same category of emissions (type of greenhouse 
gases covered, geographical / political entity). <br />The pre-filled figures refer to the global CO2 emissions 
including emissions from land use change. Regarding the global emission budget, we refer in particular to the 
IPCC Special Report 2018 (<a href = 'www.ipcc.ch/sr15/' target = '_blank'>www.ipcc.ch/sr15/</a>).<br />
<a href ='https://www.klima-retten.info/PDF/IPCC_SR15_Remaining_Carbon_Budgets.pdf', target = '_blank'>Here</a> 
we have summarized the statements of the IPCC. For the determination of national budgets, we refer to:
<ul><li>Our web app determining Paris compatible emission paths for the EU: <a href = 'http://eu.climate-calculator.info' target = '_blank'>http://eu.climate-calculator.info</a></li>
<li>Further tools for the determination of Paris compatible emission paths for all countries: <a href = 'http://www.save-the-climate.info/' target = '_blank'>http://www.save-the-climate.info/</a></li>
<li>Possible data sources: <a href = 'https://www.globalcarbonproject.org/' target = '_blank'>GCP</a>, <a href = 'https://edgar.jrc.ec.europa.eu/' target = '_blank'>EDGAR</a>, EFA</li></ul>
...
output$box_info_negative_emissions <- renderUI({
  hidden(div(class = "info-box", style = "left:390px; width:500px;", id = "info_negative_emissions", 
             HTML("An important question concerns the possibility of future negative emissions. The app allows you specify the potential 
               for net negative emissions by specifying a percentage that is applied to the current emissions. This percentage then 
               determines the minimum value of the emission paths by 2100.<br />If net negative emissions are allowed, the budget 
               may be temporarily exceeded. These overshoot amounts will then be offset by net negative emissions by 2100. However, it should be 
               noted that the overshoot amounts can also lead to dangerous <b>tipping points</b> in the climate system being exceeded. Also, it should be 
               pointed out that the <b>costs</b> of actively capturing CO2 are still unclear and that there are major methodological and substantive 
               problems in <b>quantifying</b> sinks.<br />The actual overshoot amounts
               per scenario type are displayed in the table above the emission paths.<br />"),
